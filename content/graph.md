...
.legend-chip.active { border-color: #C89F65; box-shadow: 0 2px 10px rgba(200,159,101,.35); font-weight: 600; }
...
#graph-search { width: 200px; padding: 7px 12px; border: 1px solid var(--border, #ddd); border-radius: 999px; background: var(--theme, #fff); color: var(--content, #333); font-size: 13px; outline: none; transition: border-color .15s, box-shadow .15s; }
#graph-search:focus { border-color: #C89F65; box-shadow: 0 0 0 3px rgba(200,159,101,.18); }
#graph-search-list { display: none; position: absolute; top: calc(100% + 6px); right: 0; width: 260px; max-height: 300px; overflow-y: auto; background: var(--theme, #fff); border: 1px solid var(--border, #e5e5e5); border-radius: 12px; box-shadow: 0 12px 32px rgba(0,0,0,.14); z-index: 50; }
#graph-search-list.show { display: block; }
.gs-item { display: flex; align-items: center; gap: 8px; padding: 9px 14px; cursor: pointer; font-size: 13px; color: var(--content, #333); }
...
#graph-container { width: 100%; height: 82vh; min-height: 560px; border-radius: 18px; background: var(--code-bg, #f8f8f8); border: 1px solid var(--border, #eee); position: relative; overflow: hidden; box-shadow: 0 10px 40px rgba(20, 30, 50, .06); }
...
.graph-spinner { width: 34px; height: 34px; border-radius: 50%; border: 3px solid var(--border, #e5e5e5); border-top-color: #C89F65; animation: gspin .9s linear infinite; }
...
#graph-zoomctl button { width: 34px; height: 34px; border-radius: 10px; border: 1px solid var(--border, #e5e5e5); background: var(--theme, #fff); color: var(--content, #333); font-size: 16px; cursor: pointer; transition: all .15s; box-shadow: 0 2px 10px rgba(0,0,0,.08); }
#graph-zoomctl button:hover { border-color: #C89F65; color: #C89F65; transform: translateY(-1px); }
#graph-container svg { display: block; }
.grid-dot { fill: var(--border, #e2e2e2); opacity: .5; }
.graph-link { fill: none; stroke: var(--secondary, #9aa3af); transition: stroke-opacity .2s; }
.graph-link.hl { stroke: #C89F65; stroke-opacity: .95 !important; }
.graph-link.dim { stroke-opacity: .05 !important; }
.graph-node { cursor: pointer; transition: opacity .2s; }
.graph-node.dim { opacity: .1; }
.graph-node circle.core { stroke: var(--theme, #fff); stroke-width: 1.5; transition: filter .2s; }
.graph-node.hovered circle.core, .graph-node.pinned circle.core { stroke: #C89F65; stroke-width: 2.5; filter: drop-shadow(0 0 6px rgba(200,159,101,.8)); }
...
.pulse-ring { fill: none; stroke: #C89F65; stroke-width: 2; pointer-events: none; transform-box: fill-box; transform-origin: center; animation: pulseRing 1.1s ease-out 2; }
@keyframes pulseRing { 0% { transform: scale(1); opacity: .9; } 100% { transform: scale(3.4); opacity: 0; } }
#graph-panel { position: absolute; top: 16px; right: 16px; width: 300px; max-height: calc(100% - 32px); overflow-y: auto; padding: 20px 20px 18px; border-radius: 16px; background: var(--theme, #fff); background: color-mix(in srgb, var(--theme, #fff) 92%, transparent); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); border: 1px solid var(--border, #e5e5e5); box-shadow: 0 16px 48px rgba(15, 25, 45, .16); z-index: 8; transform: translateX(calc(100% + 24px)); transition: transform .32s cubic-bezier(.22,.9,.3,1); }
#graph-panel.open { transform: translateX(0); }
#gp-close { position: absolute; top: 10px; right: 12px; width: 26px; height: 26px; border: none; background: transparent; color: var(--secondary, #999); font-size: 18px; cursor: pointer; border-radius: 8px; }
#gp-close:hover { color: var(--content, #333); background: var(--tertiary, #f0f0f0); }
...
.gp-chip { padding: 5px 10px; border-radius: 8px; border: 1px solid var(--border, #e5e5e5); background: var(--theme, #fff); color: var(--content, #333); font-size: 12px; cursor: pointer; transition: all .15s; }
.gp-chip:hover { border-color: #C89F65; color: #C89F65; transform: translateY(-1px); }
#gp-cta { display: block; margin-top: 16px; padding: 10px 0; text-align: center; border-radius: 10px; background: #1E3A5F; color: #FAF9F6 !important; font-size: 14px; font-weight: 600; text-decoration: none; transition: all .15s; }
#gp-cta:hover { background: #274a77; box-shadow: 0 6px 18px rgba(30,58,95,.3); }
#graph-tooltip { display: none; position: fixed; padding: 10px 14px; border-radius: 10px; background: #16223a; color: #fff; font-size: 13px; pointer-events: none; z-index: 100; max-width: 260px; box-shadow: 0 4px 16px rgba(0,0,0,.3); line-height: 1.6; }
...
@media (max-width: 680px) {
  ...
  #graph-panel { top: auto; bottom: 12px; left: 12px; right: 12px; width: auto; max-height: 46%; transform: translateY(calc(100% + 16px)); }
...
(function () {
  ...
  function render(data) {
    ...
    node.style('opacity', 0)
      .transition().delay(function (d, i) { return 150 + i * 5; }).duration(450)
      .style('opacity', 1)
      .on('end', function () { d3.select(this).style('opacity', null); });
    link.style('opacity', 0)
      .transition().delay(650).duration(900)
      .style('opacity', 1)
      .on('end', function () { d3.select(this).style('opacity', null); });
    ...
    function doSearch() {
      ...
      if (!matches.length) { searchList.classList.remove('show'); return; }
    ...
    searchInput.addEventListener('blur', function () {
      setTimeout(function () { searchList.classList.remove('show'); }, 150);
    });
