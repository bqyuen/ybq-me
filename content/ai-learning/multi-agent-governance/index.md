...
> **版本说明（2026-08-31）**：本文描述的"双索引 RAG"是 2026-07 的架构快照。2026-08 起知识库已简化为**单索引 + SPU 卡片索引层**——表格聚合的职责由「防爆块规则 + 排除清单」和「SPU 卡片层」承接（详见《[从零搭建本地 AI 知识库](/ai-learning/local-ai-knowledgebase/)》）。治理原则、权限矩阵与交接闭环不受影响，仍然完全有效。
...
这篇文章写于 2026-07，那时知识库是双索引架构。到 2026-08，架构演进为单索引 + SPU 卡片索引层（见《[知识库 2.0：给 10 万行采购数据建一层「卡片索引」](/ai-learning/spu-reference-fusion/)》），治理也随之调整了三处：
